...
public final class F_LockFreeSkipList<T> {
	static final int MAX_LEVEL = 4;
	final Node<T> head = new Node<T>(Integer.MIN_VALUE);
	final Node<T> tail = new Node<T>(Integer.MAX_VALUE);
	final Lock[] lock;

	public F_LockFreeSkipList() {
		lock = (ReentrantLock[])new ReentrantLock[MAX_LEVEL+1];
		for (int i = 0; i < head.next.length; i++) {
			head.next[i]= new AtomicMarkableReference<F_LockFreeSkipList.Node<T>>(tail, false);
  ...
  public T FindAndMarkMin()
  {
		...
		//try{
			...
			try{
				...
				//System.out.println("GETMAINHEAD");
				lock[0].lock();
				//System.out.println("GOTMAINHEAD");
				curr = head.next[0].getReference();
				if(curr==null || curr.value==null)return null;
				value = curr.value;
			}finally{lock[0].unlock();}

				for (int level = MAX_LEVEL;level >= 0; level--) {
					try{
						//System.out.println("GETHEAD"+level);
						lock[level].lock();
						//if(level==MAX_LEVEL){curr = head.next[level].getReference(); value = curr.value;
							//		if(curr==null || curr.value==null)return null;}
					...
					if(curr.value == value) //make head.next[level] as my next
					{
						while(true)
						{
							...
							//nextNext = next.next[level].getReference();
							//System.out.println(next.value + "next-->nextNext" + nextNext.value);
							//if(curr.next[level].compareAndSet(next,next,false,true);
							if(head.next[level].compareAndSet(curr,next,false,false))
								break;
						}
					}
				}finally{//System.out.println("LEFTHEAD"+level);
				lock[level].unlock();}
		  }
	return value;
}
